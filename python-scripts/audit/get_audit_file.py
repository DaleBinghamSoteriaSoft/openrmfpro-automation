#!/usr/bin/env python3
# ============================================================
# OpenRMF Professional External API - Audit File
# API Path   : GET /audit
# Description: Retrieves data from the /audit endpoint and saves the response body to a local Excel file.
#
# Required Parameters:
...
#
# Optional Parameters:
#    - beginDate (query), type: string, default:
#    - endDate (query), type: string, default:
#    - auditType (query), type: string, default:
#    - actionType (query), type: string, default:
#    - action (query), type: string, default:
#    - component (query), type: string, default:
#   outputFile (script option) - Local path to save the downloaded file.
#
# Command Line Example:
#   python3 get_audit_file.py \
...
def determine_output_path(response, options: dict[str, str]) -> Path:
    ...
    if filename_match:
        ...
        extension = Path(original_name).suffix or f'.{FILE_EXTENSION_HINT}'
        return downloads_root / f"full-audit_{timestamp}{extension}"

    return downloads_root / f"full-audit_{timestamp}.{FILE_EXTENSION_HINT}"
...
if 200 <= response.status_code < 300:
    output_path = determine_output_path(response, optional_arguments)
