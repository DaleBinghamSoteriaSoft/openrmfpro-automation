...
# Description: Retrieves data from the /systempackage/{systemKey}/readiness endpoint and saves the response body to a local Excel file.
#
# Required Parameters:
...
#   python3 get_systempackage_by_systemkey_readiness_json.py \
...
def determine_output_path(options: dict[str, str], system_key: str) -> Path:
    ...
    return downloads_root / f"{safe_system_key}-readiness-{timestamp}.{FILE_EXTENSION_HINT}"
