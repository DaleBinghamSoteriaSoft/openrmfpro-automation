#!/usr/bin/env python3
# ============================================================
# OpenRMF Professional External API - Systempackage Rar
# API Path   : GET /systempackage/{systemKey}/rar
# Description: Retrieves data from the /systempackage/{systemKey}/rar endpoint. The response body is saved to a local file.
#
# Required Parameters:
...
#
# Optional Parameters:
#    - fullReport (query), type: boolean, default: False
#   outputFile (script option) - Local path to save the downloaded file.
#
# Command Line Example:
#   python3 get_systempackage_by_systemkey_rar_file.py \
...
PATH_TEMPLATE = '/systempackage/{systemKey}/rar'
HTTP_METHOD = 'GET'
...
def determine_output_path(response, options: dict[str, str], system_key: str) -> Path:
    ...
    filename = f"{safe_system_key}_rar_{timestamp}.{extension}"
