#!/usr/bin/env python3
# ============================================================
# OpenRMF Professional External API - Systempackage Evidence Checklist Vuln
# API Path   : POST /systempackage/{systemKey}/evidence/checklist/{artifactId}/vuln/{vulnId}
# Description: Submits data to the /systempackage/{systemKey}/evidence/checklist/{artifactId}/vuln/{vulnId} endpoint. The script reports the HTTP status code and a human-readable meaning.
#
# Required Parameters:
...
#   5) artifactId         - Required path parameter.
#   6) vulnId             - Required path parameter.
#   7) title              - Required multipart form field (string).
#   8) evidenceFile       - Required multipart form file path.
#
# Optional Parameters:
#    - description (multipart form), type: string, default:
#    - allowOverwrite (multipart form), type: boolean, default: False
#
# Command Line Example:
#   python3 post_systempackage_by_systemkey_evidence_checklist_by_artifactid_vuln_by_vulnid.py \
...
PATH_TEMPLATE = '/systempackage/{systemKey}/evidence/checklist/{artifactId}/vuln/{vulnId}'
HTTP_METHOD = 'POST'
...
positional_values = sys.argv[4:4 + 5]
optional_values = sys.argv[4 + 5:]
