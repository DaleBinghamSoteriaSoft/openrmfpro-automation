...
# API Path   : POST /systempackage/{systemKey}/softwareasset
# Description: Submits data to the /systempackage/{systemKey}/softwareasset endpoint. The script reports the HTTP status code and a human-readable meaning.
#
# Required Parameters:
...
#   6) softwareName       - Required multipart form field (string).
#   7) softwareVersion    - Required multipart form field (string).
#   8) assetType          - Required multipart form field (string).
#   9) installedOn        - Required multipart form field (string).
#   10) approvalReference  - Required multipart form field (string).
#
# Optional Parameters:
#   None
#
# Command Line Example:
#   python3 post_systempackage_by_systemkey_softwareasset.py \
...
PATH_TEMPLATE = '/systempackage/{systemKey}/softwareasset'
HTTP_METHOD = 'POST'
