...
# API Path   : PUT /systempackage/{systemKey}/poam/{poamId}
# Description: Updates data on the /systempackage/{systemKey}/poam/{poamId} endpoint. The script reports the HTTP status code and a human-readable meaning.
#
# Required Parameters:
...
#   5) poamId             - Required path parameter.
#   6) officeorg          - Required multipart form field (string).
#   7) mitigations        - Required multipart form field (string).
#   8) comments           - Required multipart form field (string).
#   9) resources          - Required multipart form field (string).
#   10) scheduledCompletionDate - Required multipart form field (string).
#   11) milestoneCompletionDates - Required multipart form field (string).
#   12) predisposingConditions - Required multipart form field (string).
#   13) severity           - Required multipart form field (string).
#   14) relevanceOfThreat  - Required multipart form field (string).
#   15) threatDescription  - Required multipart form field (string).
#   16) likelihood         - Required multipart form field (string).
#   17) impact             - Required multipart form field (string).
#   18) impactDescription  - Required multipart form field (string).
#   19) residualRiskLevel  - Required multipart form field (string).
#   20) recommendations    - Required multipart form field (string).
#   21) residualRiskLevelMitigations - Required multipart form field (string).
#   22) tagList            - Required multipart form field (string).
#   23) riskaccepted       - Required multipart form field (boolean).
#   24) securityCheck      - Required multipart form field (string).
#   25) devices            - Required multipart form field (string).
#   26) source             - Required multipart form field (string).
#   27) rawSeverityValue   - Required multipart form field (integer).
#   28) controlVulnerabilityDescription - Required multipart form field (string).
#   29) securityControlNumbers - Required multipart form field (string).
#   30) linkedId           - Required multipart form field (string).
#   31) status             - Required multipart form field (string).
#   32) milestoneEventId   - Required multipart form field (string).
#   33) falsePositive      - Required multipart form field (boolean).
#   34) misleadingInformation - Required multipart form field (string).
#
# Optional Parameters:
#   None
#
# Command Line Example:
#   python3 put_systempackage_by_systemkey_poam_by_poamid.py \
...
PATH_TEMPLATE = '/systempackage/{systemKey}/poam/{poamId}'
HTTP_METHOD = 'PUT'
...
positional_values = sys.argv[4:4 + 31]
optional_values = sys.argv[4 + 31:]
