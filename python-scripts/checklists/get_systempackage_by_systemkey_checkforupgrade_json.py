#!/usr/bin/env python3
# ============================================================
# OpenRMF Professional External API - Systempackage Checkforupgrade
# API Path   : GET /systempackage/{systemKey}/checkforupgrade
# Description: Retrieves data from the /systempackage/{systemKey}/checkforupgrade endpoint. The response is parsed as JSON and printed with standard indentation.
#
# Required Parameters:
...
#   5) stigType           - Required query parameter.
#   6) stigVersion        - Required query parameter.
#   7) stigRelease        - Required query parameter.
#
# Command Line Example:
#   python3 get_systempackage_by_systemkey_checkforupgrade_json.py \
...
PATH_TEMPLATE = '/systempackage/{systemKey}/checkforupgrade'
HTTP_METHOD = 'GET'
...
if 200 <= response.status_code < 300:
    try:
        ...
        if isinstance(data, dict):
            data = {k: v for k, v in data.items() if k != 'rawData'}
