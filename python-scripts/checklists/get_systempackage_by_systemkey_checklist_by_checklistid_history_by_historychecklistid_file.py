#!/usr/bin/env python3
# ============================================================
# OpenRMF Professional External API - Systempackage Checklist History
# API Path   : GET /systempackage/{systemKey}/checklist/{checklistId}/history/{historyChecklistId}
# Description: Retrieves data from the /systempackage/{systemKey}/checklist/{checklistId}/history/{historyChecklistId} endpoint. The response body is saved to a local file.
#
# Required Parameters:
...
#   6) historyChecklistId - Required path parameter.
#
# Optional Parameters:
#    - format (query), type: string, default: ckl
#   outputFile (script option) - Local path to save the downloaded file.
#
# Command Line Example:
#   python3 get_systempackage_by_systemkey_checklist_by_checklistid_history_by_historychecklistid_file.py \
...
PATH_TEMPLATE = '/systempackage/{systemKey}/checklist/{checklistId}/history/{historyChecklistId}'
HTTP_METHOD = 'GET'
...
# -------------------------------------------------------
def determine_output_path(response, options: dict[str, str], system_key: str) -> Path:
    ...
    if filename_match:
        ...
        if "format" in options and options["format"].strip():
            ...
            extension = "bin"
        filename = f"{final_segment}.{extension}"
    ...
    return downloads_root / safe_system_key / filename

# -------------------------------------------------------
# Validate required arguments and map them to API parameters
# -------------------------------------------------------
minimum_argument_count = 4 + 3
...
positional_values = sys.argv[4:4 + 3]
optional_values = sys.argv[4 + 3:]
...
if 200 <= response.status_code < 300:
    output_path = determine_output_path(response, optional_arguments, path_values["systemKey"])
