...
# API Path   : PUT /systempackage/{systemKey}/ppsm/{ppsmId}
# Description: Updates data on the /systempackage/{systemKey}/ppsm/{ppsmId} endpoint. The script reports the HTTP status code and a human-readable meaning.
#
# Required Parameters:
...
#   5) ppsmId             - Required path parameter.
#   6) boundary1In        - Required multipart form field (boolean).
#   7) boundary1Out       - Required multipart form field (boolean).
#   8) boundary2In        - Required multipart form field (boolean).
#   9) boundary2Out       - Required multipart form field (boolean).
#   10) boundary3In        - Required multipart form field (boolean).
#   11) boundary3Out       - Required multipart form field (boolean).
#   12) boundary4In        - Required multipart form field (boolean).
#   13) boundary4Out       - Required multipart form field (boolean).
#   14) boundary5In        - Required multipart form field (boolean).
#   15) boundary5Out       - Required multipart form field (boolean).
#   16) boundary6In        - Required multipart form field (boolean).
#   17) boundary6Out       - Required multipart form field (boolean).
#   18) boundary7In        - Required multipart form field (boolean).
#   19) boundary7Out       - Required multipart form field (boolean).
#   20) boundary8In        - Required multipart form field (boolean).
#   21) boundary8Out       - Required multipart form field (boolean).
#
# Optional Parameters:
#   None
#
# Command Line Example:
#   python3 put_systempackage_by_systemkey_ppsm_by_ppsmid.py \
...
PATH_TEMPLATE = '/systempackage/{systemKey}/ppsm/{ppsmId}'
HTTP_METHOD = 'PUT'
...
positional_values = sys.argv[4:4 + 18]
optional_values = sys.argv[4 + 18:]
