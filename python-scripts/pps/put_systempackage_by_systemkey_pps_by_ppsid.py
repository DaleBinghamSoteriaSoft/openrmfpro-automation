#!/usr/bin/env python3
# ============================================================
# OpenRMF Professional External API - Systempackage Pps
# API Path   : PUT /systempackage/{systemKey}/pps/{ppsId}
# Description: Updates data on the /systempackage/{systemKey}/pps/{ppsId} endpoint. The script reports the HTTP status code and a human-readable meaning.
#
# Required Parameters:
...
#   5) ppsId              - Required path parameter.
#   6) title              - Required multipart form field (string).
#   7) lowPort            - Required multipart form field (integer).
#   8) highPort           - Required multipart form field (integer).
#   9) protocol           - Required multipart form field (string).
#   10) serviceName        - Required multipart form field (string).
#   11) description        - Required multipart form field (string).
#
# Optional Parameters:
#   None
#
# Command Line Example:
#   python3 put_systempackage_by_systemkey_pps_by_ppsid.py \
...
PATH_TEMPLATE = '/systempackage/{systemKey}/pps/{ppsId}'
HTTP_METHOD = 'PUT'
