...
# API Path   : POST /systempackage/{systemKey}/pps
# Description: Submits data to the /systempackage/{systemKey}/pps endpoint. The script reports the HTTP status code and a human-readable meaning.
#
# Required Parameters:
...
#   6) lowPort            - Required multipart form field (integer).
#   7) highPort           - Required multipart form field (integer).
#   8) protocol           - Required multipart form field (string).
#   9) serviceName        - Required multipart form field (string).
#   10) description        - Required multipart form field (string).
#
# Optional Parameters:
#   None
#
# Command Line Example:
#   python3 post_systempackage_by_systemkey_pps.py \
...
PATH_TEMPLATE = '/systempackage/{systemKey}/pps'
HTTP_METHOD = 'POST'
