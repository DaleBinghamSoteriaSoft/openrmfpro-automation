#!/usr/bin/env python3
# ============================================================
# OpenRMF Professional External API - Systempackage Hardwareasset
# API Path   : PUT /systempackage/{systemKey}/hardwareasset/{assetId}
# Description: Updates data on the /systempackage/{systemKey}/hardwareasset/{assetId} endpoint. The script reports the HTTP status code and a human-readable meaning.
#
# Required Parameters:
...
#   5) assetId            - Required path parameter.
#   6) deviceName         - Required multipart form field (string).
#   7) operatingSystem    - Required multipart form field (string).
#   8) firmware           - Required multipart form field (string).
#   9) manufacturer       - Required multipart form field (string).
#   10) modelNumber        - Required multipart form field (string).
#   11) assetName          - Required multipart form field (string).
#   12) purpose            - Required multipart form field (string).
#   13) description        - Required multipart form field (string).
#   14) virtualServer      - Required multipart form field (boolean).
#   15) serialNumber       - Required multipart form field (string).
#   16) tagList            - Required multipart form field (string).
#   17) profileId          - Required multipart form field (string).
#   18) ipAddresses        - Required multipart form field (string).
#   19) macAddresses       - Required multipart form field (string).
#
# Optional Parameters:
#   None
#
# Command Line Example:
#   python3 put_systempackage_by_systemkey_hardwareasset_by_assetid.py \
...
PATH_TEMPLATE = '/systempackage/{systemKey}/hardwareasset/{assetId}'
HTTP_METHOD = 'PUT'
...
positional_values = sys.argv[4:4 + 16]
optional_values = sys.argv[4 + 16:]
