...
# API Path   : POST /systempackage/{systemKey}/hardwareasset
# Description: Submits data to the /systempackage/{systemKey}/hardwareasset endpoint. The script reports the HTTP status code and a human-readable meaning.
#
# Required Parameters:
...
#   5) deviceName         - Required multipart form field (string).
#   6) operatingSystem    - Required multipart form field (string).
#   7) firmware           - Required multipart form field (string).
#   8) manufacturer       - Required multipart form field (string).
#   9) modelNumber        - Required multipart form field (string).
#   10) assetName          - Required multipart form field (string).
#   11) purpose            - Required multipart form field (string).
#   12) description        - Required multipart form field (string).
#   13) virtualServer      - Required multipart form field (boolean).
#   14) serialNumber       - Required multipart form field (string).
#   15) tagList            - Required multipart form field (string).
#   16) profileId          - Required multipart form field (string).
#   17) ipAddresses        - Required multipart form field (string).
#   18) macAddresses       - Required multipart form field (string).
#
# Optional Parameters:
#   None
#
# Command Line Example:
#   python3 post_systempackage_by_systemkey_hardwareasset.py \
...
PATH_TEMPLATE = '/systempackage/{systemKey}/hardwareasset'
HTTP_METHOD = 'POST'
...
positional_values = sys.argv[4:4 + 15]
optional_values = sys.argv[4 + 15:]
