#!/usr/bin/env python3
# ============================================================
# OpenRMF Professional External API - Systempackage Hardware File
# API Path   : GET /systempackage/{systemKey}/hardware
# Description: Retrieves data from the /systempackage/{systemKey}/hardware endpoint and saves the response body to a local Excel file.
#
# Required Parameters:
...
#
# Optional Parameters:
#    - devicename (query), type: string
#   outputFile (script option) - Local path to save the downloaded file.
#
# Command Line Example:
#   python3 get_systempackage_by_systemkey_hardware_file.py \
...
PATH_TEMPLATE = '/systempackage/{systemKey}/hardware'
HTTP_METHOD = 'GET'
...
def determine_output_path(options: dict[str, str], system_key: str) -> Path:
    ...
    return downloads_root / f"{safe_system_key}-hardware-list-{timestamp}.{FILE_EXTENSION_HINT}"
